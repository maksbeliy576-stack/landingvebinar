...
<section class="section">
  <div class="container">
    ...
    <div class="segment-grid">
      ${[
        ['Торговая компания', 'Много заявок, повторные продажи, прайсы, остатки и дебиторка. Нужен контроль менеджеров и интеграция с 1С.'],
        ['Производство', 'Длинные сделки, КП, спецификации, статусы производства и согласования между продажами, складом и бухгалтерией.'],
        ['Проектные B2B-продажи', 'Несколько ЛПР, длинный цикл сделки, задачи по касаниям, прогноз выручки и контроль этапов переговоров.'],
        ['Сервисная компания', 'Заявки из разных каналов, SLA, повторные обращения, база знаний и прозрачная история обслуживания клиента.'],
      ].map(([title, text]) => `<article class="segment-card"><span>${escapeHtml(title[0])}</span><h3>${escapeHtml(title)}</h3><p>${escapeHtml(text)}</p></article>`).join('')}
    </div>
  </div>
</section>
...
<section class="section">
  <div class="container">
    ...
    <div class="role-grid">
      ${[
        ['Сначала бизнес-логика', 'Не начинаем с кнопок и роботов: сначала разбираем процесс продаж, роли, KPI и точки потерь.'],
        ['CRM должна работать каждый день', 'Настраиваем не “для галочки”, а так, чтобы менеджеры реально вели сделки в системе.'],
        ['Результат должен быть виден', 'Делаем отчеты, план-факт, контроль этапов и понятные метрики для руководителя.'],
        ['После запуска есть развитие', 'Передаем инструкции, обучаем команду и можем сопровождать систему по backlog улучшений.'],
...
<section class="section section--white">
  <div class="container">
    ...
    ${iconFeatureGrid(serviceCopy.fit.map((item, index) => ({
      icon: ['01', '02', '03'][index],
      title: item,
      text: 'На аудите покажем, какой первый этап даст быстрый управленческий эффект и не создаст лишний объем внедрения.',
    })))}
  </div>
</section>
